...
describe("MVP1 — Q reporting copilot", () => {
  ...
  it("produces a grounded project report from live data (no invention)", async () => {
    const { markdown, usedAi } = await generateReport(kcb, {
      type: "project",
      targetId: projectId,
      tenantName: kcbName,
    });
  ...
  it("produces a resource/workload report for a person", async () => {
    const { markdown } = await generateReport(kcb, { type: "resource", targetId: kcb.userId, tenantName: kcbName });
    expect(markdown).toContain(memberName);
    expect(markdown.toLowerCase()).toContain("workload");
  });

  it("produces a portfolio summary", async () => {
    const { markdown } = await generateReport(kcb, { type: "portfolio", tenantName: kcbName });
    expect(markdown.toLowerCase()).toContain("portfolio");
  });

  it("writes an AiCallLog row (metrics only) per report", async () => {
    const logs = await withTenant(kcb, (tx) =>
      tx.aiCallLog.findMany({ where: { userId: kcb.userId }, orderBy: { createdAt: "desc" } }),
    );
  ...
  it("keeps AiCallLog tenant-isolated (RLS)", async () => {
    const seen = await withTenant(riverbank, (tx) => tx.aiCallLog.findMany({ where: { userId: kcb.userId } }));
    expect(seen).toHaveLength(0); // Riverbank cannot see KCB's Q call logs
  });

  it("errors cleanly for a missing project", async () => {
    await expect(
      generateReport(kcb, { type: "project", targetId: "does-not-exist", tenantName: kcbName }),
