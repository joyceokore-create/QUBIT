...
describe("Phase 6.1 — task keys, taxonomy, blocked flag", () => {
  ...
  beforeAll(async () => {
    ...
    demoB = { tenantId: k.id, userId: kUser.id, roles: ["ProjectManager"] };
  ...
  it("assigns sequential keys to published tasks; drafts get none until approved", async () => {
    await addTasks(demoB, projectId, [{ title: "First" }, { title: "Second", type: "Bug", severity: "High" }]);
    let tasks = await listProjectTasks(demoB, projectId);
    expect(tasks.map((t) => t.taskKey).sort()).toEqual([`${CODE}-1`, `${CODE}-2`]);
    expect(tasks.find((t) => t.title === "Second")?.type).toBe("Bug");

    await addTasks(demoB, projectId, [{ title: "AI draft" }], { approvalStatus: "Draft" });
    tasks = await listProjectTasks(demoB, projectId);
    expect(tasks.find((t) => t.title === "AI draft")?.taskKey).toBeNull();

    await publishProjectDrafts(demoB, projectId);
    tasks = await listProjectTasks(demoB, projectId);
  ...
  it("keeps keys unique under concurrent adds (counter row serializes claims)", async () => {
    await Promise.all([
      addTasks(demoB, projectId, [{ title: "C1" }, { title: "C2" }]),
      addTasks(demoB, projectId, [{ title: "C3" }]),
      addTasks(demoB, projectId, [{ title: "C4" }, { title: "C5" }]),
    ]);
  ...
  it("publishes a single draft via updateTask and claims its key", async () => {
    await addTasks(demoB, projectId, [{ title: "Solo draft" }], { approvalStatus: "Draft" });
    const draft = (await listProjectTasks(demoB, projectId)).find((t) => t.title === "Solo draft")!;
    expect(draft.taskKey).toBeNull();
    await updateTask(demoB, draft.id, { approvalStatus: "Published" });
    const published = (await listProjectTasks(demoB, projectId)).find((t) => t.title === "Solo draft")!;
    expect(published.taskKey).toBe(`${CODE}-9`);
  });

  it("flags a task blocked via a linked Open blocker, and unflags by resolving it", async () => {
    const task = (await listProjectTasks(demoB, projectId)).find((t) => t.title === "First")!;
    expect(task.blocked).toBe(false);

    await flagTaskBlocked(demoB, task.id, { description: "Waiting on vendor" });
    let after = (await listProjectTasks(demoB, projectId)).find((t) => t.id === task.id)!;
    ...
    const progress = await getProjectProgress(demoB, projectId);
    expect(progress.blocked).toBe(1);

    await unflagTaskBlocked(demoB, task.id);
    after = (await listProjectTasks(demoB, projectId)).find((t) => t.id === task.id)!;
    expect(after.blocked).toBe(false);
    expect((await getProjectProgress(demoB, projectId)).blocked).toBe(0);
  });
  ...
  it("creates a task assigned to a member and placed in a column in one step (per Joyce)", async () => {
    await addTasks(demoB, projectId, [
      { title: "Assigned bug", type: "Bug", severity: "High", status: "InQA", assigneeId: demoB.userId },
    ]);
    const t = (await listProjectTasks(demoB, projectId)).find((x) => x.title === "Assigned bug")!;
    expect(t.assigneeId).toBe(demoB.userId);
    expect(t.status).toBe("InQA");
    expect(t.type).toBe("Bug");
    await expect(
      addTasks(demoB, projectId, [{ title: "Ghost assignee", assigneeId: "00000000-0000-4000-8000-000000000000" }]),
  ...
  it("creating a project with a lead enrols them as a Project Manager member (per Joyce)", async () => {
    const p = await createProject(demoB, {
      code: `LEAD-${Date.now().toString().slice(-6)}`,
      name: "Lead enrolment test",
      type: "Project",
      priority: "Med",
      status: "Planning",
      leadUserId: demoB.userId,
    });
    try {
      const member = await withTenant(demoB, (tx) =>
        tx.projectMember.findFirst({ where: { projectId: p.id, userId: demoB.userId }, select: { role: true } }),
      );
  ...
  it("auto-generates unique project codes from the name (DM1.21)", async () => {
    ...
    const mk = (name: string) =>
      createProject(demoB, { name, type: "Project", priority: "Med", status: "Planning", leadUserId: demoB.userId });
    ...
    try {
      ...
      await addTasks(demoB, b.id, [{ title: "First task" }]);
      const [t] = await listProjectTasks(demoB, b.id);
