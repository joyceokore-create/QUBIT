...
describe("Workspace — documents + access", () => {
  ...
  it("creates and reads a document", async () => {
    const doc = await createDocument(kcb, projectId, {
      title: "Field Sales BRD",
      kind: "BRD",
      content: "# Objective\nDigitise field sales.",
    });
    const list = await listDocuments(kcb, projectId);
    expect(list.some((d) => d.id === doc.id && d.kind === "BRD")).toBe(true);
    const detail = await getDocument(kcb, doc.id);
  ...
  it("grants workspace view to a project member (no role needed), denies non-members", async () => {
    ...
    expect(await canViewProject({ ...kcb, userId: memberUserId }, projectId)).toBe(false);
    // Allocate them → now they can view.
    await setProjectMember(kcb, projectId, memberUserId, { role: "Developer" });
    expect(await canViewProject({ ...kcb, userId: memberUserId }, projectId)).toBe(true);
