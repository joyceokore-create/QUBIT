...
describe("Q mock mode", () => {
  ...
  beforeAll(async () => {
    ...
    demoB = { tenantId: k.id, userId: users[0].id, roles: [] };
    leadName = users[1].name;
    memberName = users[0].name;
    code = `QM${Date.now().toString().slice(-5)}`;
    const project = await createProject(demoB, { code, name: "Mock Chat Project", type: "Project", priority: "High", status: "AtRisk" });
    projectIds.push(project.id);
    await updateProject(demoB, project.id, { leadUserId: users[1].id });
    await setProjectMember(demoB, project.id, users[0].id, { role: "Project Manager" });
  });
  ...
  it("answers 'who is in charge of <project>' from live data", async () => {
    const res = await runQChat(demoB, { messages: [{ role: "user", content: `Who is in charge of project ${code}?` }], tenantName: "the fixture tenant" });
  ...
  it("answers 'who's on <project>' with the team", async () => {
    const res = await runQChat(demoB, { messages: [{ role: "user", content: `Who is on ${code}?` }], tenantName: "the fixture tenant" });
  ...
  it("answers a compound question (status and blockers) in one reply", async () => {
    const res = await runQChat(demoB, { messages: [{ role: "user", content: `Give me the status and blockers for ${code}` }], tenantName: "the fixture tenant" });
  ...
  it("gives an attention briefing across the portfolio", async () => {
    const res = await runQChat(demoB, { messages: [{ role: "user", content: "What needs my attention today?" }], tenantName: "the fixture tenant" });
  ...
  it("answers a superlative (who has the most work)", async () => {
    const res = await runQChat(demoB, { messages: [{ role: "user", content: "Who has the most work?" }], tenantName: "the fixture tenant" });
    expect(res.toolsUsed).toContain("list_workload");
  });

  it("answers a portfolio question when no project is named", async () => {
    const res = await runQChat(demoB, { messages: [{ role: "user", content: "How is the portfolio doing?" }], tenantName: "the fixture tenant" });
    expect(res.toolsUsed).toContain("list_projects");
  ...
  it("generates a plan from pasted text without a key", async () => {
    const plan = await generatePlan(demoB, projectIds[0], {
