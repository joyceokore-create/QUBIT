import { applyCandidates, collectCandidates, collectCheckinChase } from "@/server/nudger";
import type { JobDefinition } from "@/server/jobs/types";

/**
 * Nudger jobs (M3). `nudger` runs weekday mornings over the docs/15 §6.4 matrix;
 * `checkin-chase` runs Monday 10:00 chasing last week's unconfirmed check-ins (§7).
 * Both are idempotent twice over: the JobRun key dedupes the run, and the Nudge
...
export const checkinChase: JobDefinition = {
  ...
  async run(tx, tenant) {
    const now = new Date();
    const candidates = await collectCheckinChase(tx, now);
