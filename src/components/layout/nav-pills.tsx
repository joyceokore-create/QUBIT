...
interface NavPillsProps {
  /** Admin pill is only rendered when the viewer holds `iam:manage`. */
...
const TABS = [
  ...
  { label: "Teams", href: "/admin/teams", perm: "iam:manage" as const },
  { label: "People", href: "/people" },
  { label: "Reports", href: "/reports" },
  { label: "Admin", href: "/admin", perm: "iam:manage" as const },
];

export function NavPills({ canManageIam }: NavPillsProps) {
  const pathname = usePathname();

  return (
    <nav className="flex flex-1 gap-1">
      {TABS.filter((t) => t.perm !== "iam:manage" || canManageIam).map((tab) => {
